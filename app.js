...
function renderViewer(){
  ...
  $('#updatedLine').textContent = published
    ? 'Last updated ' + formatTimeAgo(published.__publishedAt)
    : 'Waiting for the organizer to publish results…';
  ...
  if(!board.events || board.events.length === 0){
    wrap.innerHTML = `
      <div class="empty">
        ...
        <p>Results will appear here as soon as the organizer publishes them.<br>This page checks automatically every few seconds.</p>
      </div>`;
    return;
  }
...
$('#pinSubmitBtn').addEventListener('click', () => {
  ...
  if(!storedPin){
    ...
    isAdmin = true; closePinModal(); setMode(true);
    showToast('PIN set for this device. Keep it safe.');
    return;
  }
  if(entered === storedPin){ isAdmin = true; closePinModal(); setMode(true); }
  else { $('#pinErr').textContent = 'Incorrect PIN.'; }
});

$('#resetPinLink').addEventListener('click', () => {
  if(!confirm("Reset your organizer PIN on this device? You'll set a new one next login.")) return;
  lsDel(LS_PIN);
  showToast('PIN reset for this device');
  closePinModal();
});
...
function setMode(admin){
  ...
  $('#modeToggleBtn').textContent = admin ? 'Exit to Public View' : 'Organizer Login';
  $('#livePill').style.display = admin ? 'none' : '';
  if(admin){
    stopPolling();
    renderQr();
    renderAdmin();
  ...
  pollTimer = setInterval(async () => {
    ...
    try{
      ...
      if(!published || parsed.__publishedAt !== published.__publishedAt){
        ...
        $('#updatedLine').textContent = 'Last updated ' + formatTimeAgo(published.__publishedAt);
      }
...
$('#setupBtn').addEventListener('click', async () => {
  ...
  try{
    ...
    $('#setupErr').textContent = "Couldn't reach the results server. Check your internet connection and try again.";
  }
  btn.disabled = false; btn.textContent = 'Create Results Board';
});
